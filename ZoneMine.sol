...
contract ZoneMine is Permission, IERC1155TokenReceiver {
    ...
    //万年雪霜id
    uint256 internal constant fragmentId = (uint256(3) << 248) | (uint256(2) << 184);
    ...
    */
    ...
    function _newPeriod(uint256 zoneId) internal {
        ...
        info.fragmentWeight = config.fragmentWeight;
    ...
    function _calcReward(
        ...
        for (uint256 i = 0; i != roleDuration; i++) {
            ...
            if (random <= zoneInfo.fragmentWeight && availableFragments.length > 0) {
    ...
    function withdraw() external {
        ...
        for (uint256 i = 0; i != equipments.length; i++) {
            if (equipments[i].number != 0) equip.mint(msg.sender, equipments[i]);
    ...
    function onERC1155Received(
        ...
        if (msg.sender == getAddress("Fragment")) {
            ...
            if (Operate.ZoneMine == op && fragmentId == _id) {
                //进入副本
