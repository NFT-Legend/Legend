...
import "../include/IERC1155Metadata.sol";

// A sample implementation of core ERC1155 function.
abstract contract ERC1155 is IERC1155, IERC165, IERC1155Metadata {
    ...
    */
    ...
    function uri(uint256 _id) external view override virtual returns (string memory) {
        string memory uriStr = string(abi.encodePacked(uriPrefix, _id.toString(), ".json"));
        return uriStr;
    }
    ...
    */
    function safeTransferFrom(
        ...
        _removeTokenFrom(_from, _id, _value);
        _addTokenTo(_to, _id, _value);
    ...
    /**
        ...
        After the above conditions for the transfer(s) in the batch are met, this function MUST check if `_to` is a smart contract (e.g. code size > 0). If so, it MUST call the relevant `IERC1155TokenReceiver` hook(s) on `_to` and act appropriately (see "Safe Transfer Rules" section of the standard).
        @param _from    Source address
        @param _to      Target address
        @param _ids     IDs of each token type (order and length must match _values array)
        @param _values  Transfer amounts per token type (order and length must match _ids array)
        @param _data    Additional data with no specified format, MUST be sent unaltered in call to the `IERC1155TokenReceiver` hook(s) on `_to`
    */
    function safeBatchTransferFrom(
        ...
        for (uint256 i = 0; i < _ids.length; ++i) {
            ...
            _removeTokenFrom(_from, id, value);
            _addTokenTo(_to, id, value);
        }
