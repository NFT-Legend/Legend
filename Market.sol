...
contract Market is Permission, IERC1155TokenReceiver, IERC721TokenReceiver, IERC1363Receiver, IERC165 {
    ...
    function insert(Goods memory item) internal returns (bool replaced) {
        uint256 keyIndex = _listGoods.data[item.id].id;
        if (keyIndex > 0) {
            _listGoods.data[item.id] = item;
            _listGoods.data[item.id].id = keyIndex;
            _listGoods.keys[keyIndex - 1].key = item.id;
            _listGoods.keys[keyIndex - 1].deleted = false;
            return true;
        } else {
            keyIndex = _listGoods.keys.length;
            _listGoods.keys.push();
            item.id = keyIndex + 1;
            _listGoods.data[item.id] = item;
            _listGoods.keys[keyIndex].key = item.id;
            _listGoods.size++;
            return false;
        }
    }
    ...
    function next(uint256 goodsId) public view returns (uint256) {
        goodsId++;
        while (goodsId < _listGoods.keys.length && _listGoods.keys[goodsId].deleted) goodsId++;
        return goodsId;
    }
    ...
    function getGoods(uint256 goodsId) public view returns (Goods memory goods) {
        uint256 key = _listGoods.keys[goodsId - 1].key;
        goods = _listGoods.data[key];
    }

    function searchGoods(uint8 class, uint32 count) public view returns (bool more, Goods[] memory list) {
        list = new Goods[](count);
        uint256 j;
        Goods storage goods;
        for (uint256 i = next(0); j <= count && valid(i); i = next(i)) {
    ...
    function _buy(
        ...
        if (refAddr != address(0)) {
            ...
            comm = commission;
            if (ref != address(0) && commission < totalFee - chargeFee) {
                //支付佣金
        ...
        if (payContract == getAddress("Token")) {
            //销毁
            IERC20OP(payContract).burn(totalFee - comm);
